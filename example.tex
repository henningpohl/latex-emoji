...
When using XeTeX, you can also specify a \verb|text| option to set the emoji in a given font. The font to use is passed to the package during loading. On Windows, for example, you might want to use \textit{Segeo UI Emoji}. That font would be selected by using: \verb|usepackage[font=seguiemj.ttf]{emoji}|. Emojis rendered via a font are, e.g.: 
\begin{text-emojis}
\dogline
\end{text-emojis}
